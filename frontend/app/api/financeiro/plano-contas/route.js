...
export async function PUT(request) {
  try {
    ...
    const contaAtual = await turso.execute({
      sql: "SELECT id, empresa_id FROM fin_plano_contas WHERE id = ?",
      args: [id],
    });

    if (!contaAtual.rows?.length) {
      return NextResponse.json(
    ...
    if (campos.tipo_gasto) campos.tipo_gasto = normalizarTexto(campos.tipo_gasto);

    const empresaValidacao =
      campos.empresa_id ?? contaAtual.rows[0].empresa_id ?? null;

    if (campos.codigo_conta) {
      const duplicada = await turso.execute({
        sql: `SELECT id FROM fin_plano_contas
              WHERE codigo_conta = ?
              AND IFNULL(empresa_id, 0) = IFNULL(?, 0)
              AND id <> ?`,
        args: [campos.codigo_conta, empresaValidacao, id],
      });
